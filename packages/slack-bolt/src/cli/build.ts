...
export async function executeBuild(
  ...
  let configAuth: Awaited<ReturnType<typeof authTest>> | undefined;
  try {
    configAuth = await authTest({ token: params.slackConfigurationToken });
  } catch (error) {
  ...
  if (params.slackServiceToken) {
    try {
      const serviceAuth = await authTest({
        token: params.slackServiceToken,
      });
      if (configAuth && serviceAuth.userId !== configAuth.userId) {
        log.warning(
          "SLACK_CONFIGURATION_TOKEN and SLACK_SERVICE_TOKEN belong to different users. " +
            "This may cause app installation to fail. " +
